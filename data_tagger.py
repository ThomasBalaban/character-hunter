...
class DataTagger:
    ...
    def process_and_save(self, img, search_info):
        ...
        try:
            ...
            # Save image
            cv2.imwrite(str(char_dir / img_filename), cv2.cvtColor(processed_img, cv2.COLOR_RGB2BGR))
            
            # Create and save metadata
            metadata = self._create_metadata(search_info)
            with open(char_dir / meta_filename, 'w') as f:
                json.dump(metadata, f, indent=2)
            
            # Update status
            self.status_window.update_status(f"Saved image for '{character}'")
            logger.info(f"Saved image for '{character}' in '{char_dir}'")
            ...
            self.status_window.update_status("Error saving image")